...
class Cropper:
    def __init__(self,path:Path,config:LibraryConfig):
        model=Model()
        self.image_path=path
        self.image_name = Path(path).stem
        self.results=model.predict(path)
        self.save_dir=Path(config.crop_save_dir)
    ...
    def crop(self):
        ...
        for i, box in enumerate(boxes):
            if conf[i]>0.7:
                ...
                path=f"{self.image_name}_crop_{i}.jpg"
